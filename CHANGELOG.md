...
- 🐛 Remove the flash after opening a gallery photo with View Transitions

### Maintenance

- ⬆️ Upgrade Vitest and its V8 coverage provider to v4

## [1.15.1] - 2026-07-15

### Bug Fixes

- 🐛 Align Docker builder with Go 1.23
...
- Merge pull request #35 from danibram/codex/remove-native-bulk-check

♻️ Remove the never-activating native bulk-check path
...
Reverts the capability ladder added in #34. Immich is an external
dependency we don't change, so the native branch could never activate
against any server we target — it was dead code whose only effect was
making the upload-check handler harder to reason about. The checksum
probes are the one and only mechanism.
